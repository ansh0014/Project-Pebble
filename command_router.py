...
# === ROUTER ===
# === HISTORY COMMANDS ===
def handle_show_history(history_manager):
    if history_manager.show_history():
        say("Here are your recent conversations")
    else:
        say("No history found")

def handle_search_history(history_manager, command):
    search_term = command.replace("search history", "").strip()
    results = history_manager.search_history(search_term)
    if results > 0:
        say(f"Found {results} matching conversations")
    else:
        say("No matching conversations found")

def handle_clear_history(history_manager):
    say("Are you sure you want to clear history? Say yes to confirm or no to cancel")
    confirmation = takecommand()
    
    if confirmation and confirmation.lower() in ['yes', 'yeah', 'sure', 'ok', 'confirm']:
        if history_manager.delete_history():
            say("History has been cleared successfully")
        else:
            say("Failed to clear history")
    else:
        say("History deletion cancelled")

def run_command(command: str, history_manager=None):
    command = command.lower()

    # Handle history commands first
    if history_manager:
        if "show history" in command or "i want to see history" in command:
            return handle_show_history(history_manager)
        elif "search history" in command or "find history" in command or "i want to search history" in command:
            return handle_search_history(history_manager, command)
        elif any(phrase in command for phrase in ["clear history", "delete history", "remove history"]):
            return handle_clear_history(history_manager)
